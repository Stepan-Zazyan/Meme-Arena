...
class CoreMvpIntegrationTests {
    ...
    @Test void guestMemeBattleVoteAndTopFlow() throws Exception {
        String u=mvc.perform(post("/api/v1/users/guest").contentType(MediaType.APPLICATION_JSON).content("{\"nickname\":\"Stepan\"}"))
                .andExpect(status().isCreated()).andExpect(header().exists("X-Request-Id")).andExpect(jsonPath("$.user.status").value("NEWBIE")).andReturn().getResponse().getContentAsString();
        String userId=extractPath(u,"user","id"); String token=extract(u,"accessToken");
        ...
        String m=mvc.perform(post("/api/v1/memes").header("Authorization","Bearer "+token).contentType(MediaType.APPLICATION_JSON).content("{\"uploaderId\":\""+userId+"\",\"title\":\"New meme\",\"imageUrl\":\"https://example.com/m.jpg\"}"))
        ...
        String b=mvc.perform(get("/api/v1/battles/next").header("Authorization","Bearer "+token).param("userId",userId)).andExpect(status().isOk()).andExpect(jsonPath("$.left.id").exists()).andExpect(jsonPath("$.right.id").exists()).andReturn().getResponse().getContentAsString();
        String left=extractPath(b,"left","id"), right=extractPath(b,"right","id");
        mvc.perform(post("/api/v1/votes").header("Authorization","Bearer "+token).contentType(MediaType.APPLICATION_JSON).content("{\"userId\":\""+userId+"\",\"leftMemeId\":\""+left+"\",\"rightMemeId\":\""+right+"\",\"winnerMemeId\":\""+ UUID.randomUUID()+"\"}")) .andExpect(status().isBadRequest()).andExpect(jsonPath("$.code").value("INVALID_WINNER"));
        mvc.perform(post("/api/v1/votes").header("Authorization","Bearer "+token).contentType(MediaType.APPLICATION_JSON).content("{\"userId\":\""+userId+"\",\"leftMemeId\":\""+left+"\",\"rightMemeId\":\""+right+"\",\"winnerMemeId\":\""+left+"\"}")) .andExpect(status().isCreated()).andExpect(jsonPath("$.winner.ratingAfter").value(1516));
        mvc.perform(post("/api/v1/votes").header("Authorization","Bearer "+token).contentType(MediaType.APPLICATION_JSON).content("{\"userId\":\""+userId+"\",\"leftMemeId\":\""+left+"\",\"rightMemeId\":\""+right+"\",\"winnerMemeId\":\""+left+"\"}")) .andExpect(status().isConflict()).andExpect(jsonPath("$.code").value("PAIR_ALREADY_VOTED"));
